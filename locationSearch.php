...
$sTerm="";
if(!empty($_GET['sTerm'])){
	$sTerm=strToLower($_GET['sTerm']);
}

$sql = "SELECT id,name,address,city, zip, phone,price, rating FROM resturants WHERE lcase(city) LIKE '%$sTerm%' order by rating desc";
...
echo "Here are the restaurants in $sTerm:<br><br>";
$rows = array();
for($i=0;$i<$count;$i++){
